    ...
    return html`
      ...
      <app-header condenses reveals effects="waterfall">
        <app-toolbar class="toolbar-top">
          <button class="menu-btn" aria-label="Menu" ?hidden="${hideMenuBtn}"
              @click="${() => store.dispatch(updateDrawerState(true))}"><img src="https://cdn4.iconfinder.com/data/icons/wirecons-free-vector-icons/32/menu-alt-512.png" width="40" /></button>
          <a class="back-btn" aria-label="Go back" ?hidden="${!hideMenuBtn}" href="${backHref}">${backIcon}</a>
          <div main-title><a href="/"><img width="200" style="margin-top: 8%;" src="images/kzakteb-02-1.png"/ ></a></div>
      ...
      <app-drawer .opened="${_drawerOpened}" ?hidden="${!_lazyResourcesLoaded}"
        ...
        <nav class="drawer-list" @click="${e => store.dispatch(updateDrawerState(false))}">
          <a ?selected="${_page === 'explore'}" href="/explore?q=${query}">الرئيسية	</a>
          <a ?selected="${_page === 'favorites'}" href="/favorites">المفضلة</a>
          <a ?selected="${_page === 'about'}" href="/about">من نحن</a>
        </nav>
      </app-drawer>
      ...
      <footer>
        <p>جميع الحقوق محفوظة لموقع كزاكتاب @2019 </p>
      </footer>
